...
One method that can be used to solve the transport equation is the Spherical Harmonics (P$_n$) method\todo{citation}.  This method is able to resolve the angle dependence of the angular flux, but is very complicated and cumbersome to implement.  A more efficient method that is employed in MPACT is the Simplified Spherical Harmonics (SP$_n$) method.  This method is identical to the P$_n$ method in 1D, but greatly simplifies the equations to be solved in 2D and 3D while maintaining a high level of accuracy.  The derivation below will focus only on the SP$_n$ method in one spatial dimension, since this is how it is used in MPACT.
